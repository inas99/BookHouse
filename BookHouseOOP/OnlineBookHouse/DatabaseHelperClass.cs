...
namespace BookHouseOOP
{
    ...
    {
        private SqlConnection connection = new SqlConnection(@"Server=tcp:dboopsql.database.windows.net,1433;Initial Catalog=BookHouse;Persist Security Info=False;User ID=myadmin;Password=Inas2020;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;");
        ...
        public void removeSelectedItemsFromShoppingCart(string name)
        {
            ...
            {
                FileWriterClass.WriteFile(AppConstants.EXCEPTION_LOG_FILE_LOCATION, "Exception at removeSelectedItemsFromShoppingCart function" + e.Message);
                Console.WriteLine("Click {0}", e.Message);
            }
        }
        ...
        public BookClass getABookFromDBByID(string id)
        {
            ...
            {
                FileWriterClass.WriteFile(AppConstants.EXCEPTION_LOG_FILE_LOCATION, "Exception at getABookFromDBByID function" + e.Message);
                Console.WriteLine("Click {0}", e.Message);
                return null;
            }
        }
        ...
        public bool addAProductToDB(ProductClass product)
        {
            try
            {
                ...
                if (product.GetType() == typeof(BookHouseOOP.BookClass))
                {
                ...
                else if (product.GetType() == typeof(BookHouseOOP.MagazineClass))
                {
                ...
                else if (product.GetType() == typeof(BookHouseOOP.MusicCDsClass))
                {
        ...
        public bool deleteAnItemObjectFromDBByID(Type type, string id)
        {
            ...
            {
                FileWriterClass.WriteFile(AppConstants.EXCEPTION_LOG_FILE_LOCATION, "Exception at deleteAnItemObjectFromDBByID function" + e.Message);
                Console.WriteLine("Click {0}", e.Message);
            }
            SqlConnection connection = dbHelper.getConnection();
            SqlCommand deleteCommand = null;
            if (type == typeof(BookHouseOOP.UserClass))
                deleteCommand = new SqlCommand("delete from CustomerTable where id=@id", connection);
            if (type == typeof(BookHouseOOP.BookClass))
                deleteCommand = new SqlCommand("delete from BookTable where id=@id", connection);
            else if (type == typeof(BookHouseOOP.MagazineClass))
                deleteCommand = new SqlCommand("delete from MagazineTable where id=@id", connection);
            else if (type == typeof(BookHouseOOP.MusicCDsClass))
        ...
        public bool updateAProductAtDB(ProductClass product)
        {
            ...
            {
                FileWriterClass.WriteFile(AppConstants.EXCEPTION_LOG_FILE_LOCATION, "Exception at updateAProductAtDB function" + e.Message);
                Console.WriteLine("Click {0}", e.Message);
                return false;
            }
        }
        ...
        public DataTable getAllItemsForAClassFromATableByItsType(Type type)
        {
            ...
            try
            {
                ...
                else if (type == typeof(BookHouseOOP.BookClass))
                {
            ...
            {
                FileWriterClass.WriteFile(AppConstants.EXCEPTION_LOG_FILE_LOCATION, "Exception at getAllItemsForAClassFromATableByItsType function" + e.Message);
                Console.WriteLine("Click {0}", e.Message);
                return null;
            }
        }
        ...
        public bool confirmAUser(string id)
        {
            ...
            {
                FileWriterClass.WriteFile(AppConstants.EXCEPTION_LOG_FILE_LOCATION, "Exception at confirmAUser function" + e.Message);
                Console.WriteLine("Click {0}", e.Message);
                return false;
            }
        }
